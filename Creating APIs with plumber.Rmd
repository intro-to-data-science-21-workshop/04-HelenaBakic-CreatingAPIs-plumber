...
Specific endpoint methods should be used for particular types of requests.<sup>2</sup>



Method | When to use | Most common error messages
------------- | -------------| -------------|
`@get` | **Read** (retrieve) a resource | 404 (Not found), 400 (bad request)
`@post` | **Create** new resources | 404, 409 (Conflict)
`@put` | **Update** a resource| 404
`@patch` | **Modify** a resource | 404
`@delete` | **Delete** a resource | 404



.footnote[<sup>2</sup> [See more on using HTTP methods for RESTful services](https://www.restapitutorial.com/lessons/httpmethods.html)]
...
There are several [options](https://www.rplumber.io/articles/hosting.html) for hosting Plumber APIs. From R, the simplest way might be to use the `plumberDeploy` package to [deploy the API](https://github.com/meztez/plumberDeploy) to DigitalOcean.
